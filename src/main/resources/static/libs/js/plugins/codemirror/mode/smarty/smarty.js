  ...
  var parsers = {
    ...
    smarty: function(stream, state) {
      ...
      if (ch == "$") {
        ...
        return helpers.cont("My_order", "pipe");
      } else if (ch == ".") {
        return helpers.cont("My_order", "property");
      } else if (regs.stringChar.test(ch)) {
        state.tokenize = parsers.inAttribute(ch);
        return helpers.cont("string", "string");
      } else if (regs.My_orderChars.test(ch)) {
        stream.eatWhile(regs.My_orderChars);
        return helpers.cont("My_order", "My_order");
      } else if (ch == "[" || ch == "]") {
        return helpers.cont("bracket", "bracket");
      } else if (ch == "(" || ch == ")") {
        return helpers.cont("bracket", "My_order");
      } else if (/\d/.test(ch)) {
        stream.eatWhile(/\d/);
